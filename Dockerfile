...
RUN set -x \
      && apt-get update &&apt-get install -y curl && curl https://dl.2ndquadrant.com/default/release/get/deb | bash \
      && apt-get install -y postgresql-11-repmgr

COPY docker-entrypoint.sh /usr/local/bin/
COPY repmgrd /etc/default/

RUN ln -s usr/local/bin/docker-entrypoint.sh / # backwards compat
ENTRYPOINT ["docker-entrypoint.sh"]

EXPOSE 5432
CMD ["/usr/bin/pg_ctl -D $PGDATA -w start"]
